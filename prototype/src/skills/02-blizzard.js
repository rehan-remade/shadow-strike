...
const SKILL = (function () {
  const MAGE = sprite([
    ...
    'oFFFFFFFFFFFFo',
    '...okko.okko..',
  ], { o: 0, H: C.robeB1, h: C.robeB2, F: C.ice0, w: C.hair0, s: C.skin, S: C.skinD, e: 0, R: C.robeB1, r: C.robeB2, g: C.gold, k: C.robeB3 });
  const TOP = GY - 22;
  ...
  return {
    ...
    drawActor(k, p) {
      ...
      if (!casting) staff();
      // back arm
      line(bkx, bky, bkx + shown.bx, bky + shown.by, C.robeB3);
      ...
      if (bob) px(bx0 + 1, by0 + 21, C.robeB2);
      if (casting) staff();
      // front arm + hand
      line(fx, fy, hx, hy, C.robeB2); line(fx, fy + 1, hx, hy + 1, C.robeB1);
