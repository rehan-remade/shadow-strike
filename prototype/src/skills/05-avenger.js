...
const SKILL = (function () {
  ...
  const NINJA = sprite([
    ...
    'odno......odnno.',
    'okko......okkkko',
  ], { o: 0, N: C.ninja0, n: C.ninja1, d: C.ninja2, k: C.ninja3, s: C.skin, e: C.navy, r: C.crim2, R: C.crim1 });
  ...
  const CLONE_MAP = remapOf([[0, C.vio5], [C.ninja0, C.vio3], [C.ninja1, C.vio4], [C.ninja2, C.vio5], [C.ninja3, 0], [C.skin, C.vio3], [C.navy, C.vio0],
    [C.crim1, C.vio2], [C.crim2, C.vio3], [C.crim3, C.vio4], [C.steel, C.vio1], [35, C.vio0], [C.armor1, C.vio2], [C.armor2, C.vio3]]);
  const GHOST_MAP = remapOf([[0, C.vio3], [C.ninja0, C.vio0], [C.ninja1, C.vio1], [C.ninja2, C.vio2], [C.ninja3, C.vio3], [C.skin, C.vio0], [C.navy, 35],
  ...
  function drawNinja(s, x, y, rim) {
    ...
    PL(bsx, bsy, bhx, bhy, C.ninja2); P(bhx, bhy, C.ninja3);
    // body
    for (let r = 0; r < NR_; r++) {
    ...
    PL(fsx, fsy + 1, fhx, fhy + 1, C.ninja2);
    PL(fsx, fsy, fhx, fhy, C.ninja1);
    P(fhx, fhy, C.ninja3); P(fhx, fhy + 1, C.ninja3);
  ...
  return {
    ...
    drawActor(k, p) {
      if (actVis && !actGhost) {
        const s = snap();
        drawNinja(s, actX, actY, glow > 0.3 ? ((tick >> 2) & 1 ? C.vio1 : C.vio2) : (fireGlow() > 0.9 ? C.ninja0 : -1));
      }
    },
    drawFx(k, p) {
